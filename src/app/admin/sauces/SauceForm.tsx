...
export default function SauceForm({ initialData }: { initialData?: SauceData }) {
  ...
  const [form, setForm] = useState({
    ...
    heat_level: initialData?.heat_level != null ? String(initialData.heat_level) : '',
    ingredients: toCommaString(initialData?.ingredients),
    volume_oz: initialData?.volume_oz != null ? String(initialData.volume_oz) : '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        heat_level: form.heat_level !== '' ? parseInt(form.heat_level, 10) : null,
        ingredients: form.ingredients ? form.ingredients.split(',').map(s => s.trim()).filter(Boolean) : null,
        volume_oz: form.volume_oz !== '' ? parseFloat(form.volume_oz) : null,
      ...
      const supabase = createClient()
      if (initialData?.id) {
        const { error } = await supabase.from('sauces').update(payload).eq('id', initialData.id)
        if (error) throw error
      } else {
        const { error } = await supabase.from('sauces').insert(payload)
        if (error) throw error
      }

      router.push('/admin/sauces')
      router.refresh()
    } catch (err: unknown) {
      const msg =
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: '680px' }}>
      ...
      <Section title="Sauce details">
        <Field label="Heat level">
          <select name="heat_level" value={form.heat_level} onChange={handleChange} style={inputStyle}>
            ...
            <option value="9">9 — Very hot</option>
            <option value="10">10 — Extra hot</option>
          </select>
        </Field>
        <Field label="Volume (oz)">
          <input name="volume_oz" type="number" step="0.1" min="0" value={form.volume_oz} onChange={handleChange} style={{ ...inputStyle, width: '120px' }} />
        </Field>
        <Field label="Ingredients (comma-separated)">
          <textarea name="ingredients" value={form.ingredients} onChange={handleChange} rows={3} style={{ ...inputStyle, resize: 'vertical' }} />
        </Field>
      </Section>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
        ...
        <button
          type="button"
          onClick={() => router.push('/admin/sauces')}
